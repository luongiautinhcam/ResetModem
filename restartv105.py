...
class ModemResetApp:
    ...
    def __init__(self, root: tk.Tk):
        ...
        self.app_width, self.app_height = 450, 350
        ...
        # App state
        self.public_ip = STATUS_GETTING_IP
    ...
    def _position_window(self):
        """Restores the saved window size and position, or centers the default size."""
        self.root.update_idletasks()
        window_width = max(400, self.last_window_width or self.app_width)
        window_height = max(300, self.last_window_height or self.app_height)
        ...
        if self.last_pos_x is not None and self.last_pos_y is not None:
            ...
            if 0 <= self.last_pos_x < (screen_w - 50) and 0 <= self.last_pos_y < (screen_h - 50):
                logging.info(
                    f"Restoring window geometry to {window_width}x{window_height}"
                    f"+{self.last_pos_x}+{self.last_pos_y}"
                )
                self.root.geometry(
                    f"{window_width}x{window_height}+{self.last_pos_x}+{self.last_pos_y}"
                )
                return
    ...
    def _load_config(self):
        ...
        try:
            ...
            if self.config.has_section(CONFIG_SECTION_WINDOW):
                ...
                self.last_window_width = self.config.getint(CONFIG_SECTION_WINDOW, CONFIG_KEY_WIDTH, fallback=None)
                self.last_window_height = self.config.getint(CONFIG_SECTION_WINDOW, CONFIG_KEY_HEIGHT, fallback=None)
    ...
    def _save_config(self):
        ...
        try:
            ...
            if self.root and self.root.winfo_exists():
                ...
                self.config.set(CONFIG_SECTION_WINDOW, CONFIG_KEY_WIDTH, str(window_width))
                self.config.set(CONFIG_SECTION_WINDOW, CONFIG_KEY_HEIGHT, str(window_height))
    ...
    def _update_log_treeview(self):
        """Add a new entry to the log Treeview, handling duplicate timestamps and logical order."""
        # Exit if GUI is not available
        if not (self.root and self.root.winfo_exists() and self.tree and self.tree.winfo_exists()):
            return

        now = datetime.now().strftime("%d/%m/%Y %H:%M:%S")
        # Gather current log state
        with self.lock:
        ...
        # Determine the log status to display
        if action and action != STATUS_NORMAL:
            log_status = action
        elif connected is None and not resetting:
            log_status = STATUS_CHECKING
        elif connected:
            log_status = STATUS_NORMAL
        elif not connected and not resetting:
